...
public class WSTest  extends TestCase {
    ...
    public void testGetAllItems() {
        try {
            ...
            assertEquals("Price", new Float(10), items[1].getPrice());
            assertEquals("Price Vector", new Float(10), 
                    ((ItemPriceDTOEx) items[1].getPrices().get(0)).getPrice());
            assertEquals("ID", new Integer(1), items[1].getId());
            assertEquals("Number", "DP-1", items[1].getNumber());
            ...
            assertEquals("Price", new Float(20), items[2].getPrice());
            assertEquals("Price Vector", new Float(20), 
                    ((ItemPriceDTOEx) items[2].getPrices().get(0)).getPrice());
            assertEquals("ID", new Integer(2), items[2].getId());
            assertEquals("Number", "DP-2", items[2].getNumber());
            ...
            assertEquals("Price", new Float(15), items[3].getPrice());
            assertEquals("Price Vector", new Float(15), 
                    ((ItemPriceDTOEx) items[3].getPrices().get(0)).getPrice());
            assertEquals("ID", new Integer(3), items[3].getId());
            assertEquals("Number", "DP-3", items[3].getNumber());
            assertEquals("Type 1", new Integer(1), items[3].getTypes()[0]);

            assertEquals("Description", "10% Elf discount.", 
                    items[4].getDescription());
            assertEquals("Percentage", new Float(-10.00), 
                    items[4].getPercentage());
            assertEquals("ID", new Integer(14), items[4].getId());
            assertEquals("Number", "J-01", items[4].getNumber());
            assertEquals("Type 12", new Integer(12), items[4].getTypes()[0]);

            assertEquals("Description", "an item from ws", 
                    items[0].getDescription());
            assertEquals("Price", new Float(29.5), items[0].getPrice());
            assertEquals("Price Vector", new Float(29.5), 
                    ((ItemPriceDTOEx) items[0].getPrices().get(0)).getPrice());
            assertEquals("ID", new Integer(24), items[0].getId());

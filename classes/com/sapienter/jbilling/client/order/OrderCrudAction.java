...
public class OrderCrudAction extends CrudActionBase<OrderDTO> {
	...
	protected ForwardAndMessage doSetup() throws RemoteException {
		OrderDTO dto = (OrderDTO) session
				.getAttribute(Constants.SESSION_ORDER_DTO);
		...
		myForm
				.set(FIELD_DUE_DATE_VALUE, getStringOrNull(dto
						.getDueDateValue()));
		myForm.set(FIELD_DF_FM, isIntegerTrue(dto.getDfFm()));
		myForm.set(FIELD_OWN_INVOICE, isIntegerTrue(dto.getOwnInvoice()));
		myForm.set(FIELD_ADD_NOTES_IN_INVOICE, isIntegerTrue(dto
				.getNotesInInvoice()));
		myForm.set(FIELD_NOTES, dto.getNotes());
		myForm.set(FIELD_ANTICIPATE_PERIODS, getStringOrNull(dto
	...
	protected OrderDTO doEditFormToDTO() throws RemoteException {
		...
		OrderDTO summary = (OrderDTO) session
				.getAttribute(Constants.SESSION_ORDER_SUMMARY);

		OrderPeriodDTO period = new OrderPeriodDTO();
		period.setId((Integer) myForm.get(FIELD_PERIOD));
		summary.setOrderPeriod(period);
		summary.setCycleStarts(parseDate(FIELD_GROUP_CYCLE,
				"order.prompt.cycleStart"));
		summary.setActiveSince(parseDate(FIELD_GROUP_SINCE,
				"order.prompt.activeSince"));
		summary.setActiveUntil(parseDate(FIELD_GROUP_UNTIL,
		...
		summary
				.setAnticipatePeriods(getIntegerFieldValue(FIELD_ANTICIPATE_PERIODS));
		summary.setPeriodStr(getOptionDescription(summary.getPeriodId(),
				Constants.PAGE_ORDER_PERIODS));
		summary.setBillingTypeStr(getOptionDescription(summary
				.getBillingTypeId(), Constants.PAGE_BILLING_TYPE));
		...
		if (isIntegerTrue(summary.getNotify())
		...
		OrderDTO orderDTO = (OrderDTO) session
				.getAttribute(Constants.SESSION_ORDER_DTO);
		...
		if (summary.getActiveUntil() != null) {
			...
			try {
				...
				UserSessionHome userHome = (UserSessionHome) EJBFactory
						.lookUpHome(UserSessionHome.class,
			...
			boolean useProrating = Integer.parseInt(userSession
			...
			if (!useProrating
				...
				try {
					...
					myPeriod = orderSession.getPeriod(languageId, summary
							.getPeriodId());
				} catch (Exception e) {
		...
		if (summary.getPromoCode() != null
			...
			try {
				...
				ItemSessionHome itemHome = (ItemSessionHome) EJBFactory
						.lookUpHome(ItemSessionHome.class,
				...
				if (promotion == null) {
					addError("promotion.error.noExist",
				...
				if (summary.getId() != null
						|| itemSession.promotionIsAvailable(promotion.getId(),
								summary.getUserId(), promotion.getCode())
								.booleanValue()) {

					summary = myNewOrderSession.addItem(promotion.getItemId(),
							1, summary.getUserId(), entityId);

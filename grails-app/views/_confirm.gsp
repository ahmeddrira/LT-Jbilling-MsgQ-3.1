<%--
    ...
    The rendered confirmation dialog can be shown hidden using the element ID "confirm-dialog-${id}" where the
    ID value is that of the passed ID value of the action.

    To open, use "$('#confirm-dialog-123'}').dialog('open');" or the shorthand "showConfirm(123);"
 --%>

<div id="confirm-dialog-${id}" class="bg-lightbox" title="${message(code: 'popup.confirm.title')}">
    <!-- command form -->
    <g:if test="${ajax}">
        <g:formRemote name="confirm-command-form-${id}" url="[controller: controller, action: action, id: id]" update="${update}">
    ...
    <g:else>
        <g:form name="confirm-command-form-${id}" url="[controller: controller, action: action, id: id]" >
            <g:hiddenField name="id" value="${id}"/>
            <g:each var="param" in="${formParams?.entrySet()}">
                <g:if test="${param.value != null && !param.value == 'null' }">
                    <g:hiddenField name="${param.key}" value="${param.value}"/>
    ...
    $('#confirm-dialog-${id}').dialog({
        ...
        buttons: {
            "${message(code: 'prompt.yes')}": function() {
                ${onYes};
                $("#confirm-command-form-${id}").submit();
                $(this).dialog('close');
            },

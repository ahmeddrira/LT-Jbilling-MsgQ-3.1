...
public class WSTest extends TestCase {
    ...
    public void testGenerateInvoicePartialPeriod() throws Exception {
        ...
        contact.setEmail("test@test.com");
        contact.setFirstName("Invoice Test");
        contact.setLastName("Partial period (pro-rating)");
        user.setContact(contact);
        ...
        InvoiceWS invoice = api.getInvoiceWS(invoiceIds[0]);
        assertEquals("due date 45 days from order start date", new DateMidnight(2011, 3, 6).toDate(), invoice.getDueDate());
    ...
    public void testGenerateInvoiceMultiplePartialPeriod() throws Exception {
        ...
        contact.setEmail("test@test.com");
        contact.setFirstName("Invoice Test");
        contact.setLastName("Multi-order pro-rating");
        user.setContact(contact);
        ...
        order1.setCycleStarts(new DateMidnight(2011, 2, 1).toDate());     // cycle starts February 01, 2011
        ...
        order1.setId(api.createOrder(order1)); // create order
        ...
        order3.setCycleStarts(new DateMidnight(2011, 2, 1).toDate());     // cycle starts February 01, 2011
        ...
        order4.setBillingTypeId(Constants.ORDER_BILLING_PRE_PAID);
        ...
        order4.setCycleStarts(new DateMidnight(2011, 2, 1).toDate());     // cycle starts February 01, 2011

        // create an order to be invoiced
        OrderLineWS line4 = new OrderLineWS();
        line4.setTypeId(Constants.ORDER_LINE_TYPE_ITEM);
        line4.setItemId(2602); // lemonade
        line4.setUseItem(true);
        line4.setQuantity(1);
        order4.setOrderLines(new OrderLineWS[] {line4});

        order4.setId(api.createOrder(order4)); // create order
        ...
        // order date will be used when calculating invoice due date:
        //
        //      EARLIEST order starts on February 10, 2011
        //      invoiced on February 01, 2011
        //      45 day due date
        //
        // pre paid == invoice immediately, use order start date
        // February 10, 2011 + 45 = March 27, 2011
        Date billingDate = new DateMidnight(2011, 2, 1).toDate();
        Integer[] invoiceIds = api.createInvoiceWithDate(user.getUserId(), billingDate, PeriodUnitDTO.DAY, 45, false);
        assertEquals("1 invoice generated", 1, invoiceIds.length);

        InvoiceWS invoice = api.getInvoiceWS(invoiceIds[0]);
        assertEquals("due date 45 days from order start date", new DateMidnight(2011, 3, 27).toDate(), invoice.getDueDate());
